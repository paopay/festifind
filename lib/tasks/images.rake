...
module Google
	def self.start_search_for_images
		Festival.all.each do |festival|
			Google.get_json_from_google(festival.display_name)
		end
	end

	def self.get_json_from_google(display_name)
		display_name	= display_name.downcase.tr!(" ", "_")
		response = HTTParty.get("https://www.googleapis.com/customsearch/v1?key=AIzaSyA8K33aJWJrBP23XfrVn34HYg2fG4Js1AI&cx=001913588589495655496:k2ycnxpl5ye&q=#{display_name}+lineup+poster&searchType=image&imgSize=large&alt=json&limit=1")
		ap response["items"][0]["link"]
	end
